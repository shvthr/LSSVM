function [alpha,b,Yt] = modridgeregress(X,Y,gam,kernel_type,sig,svX,omega,Xt)
...
if (nD<50000)
    H = [X ones(nD,1)]'*[X ones(nD,1)] + inv(gam).*[omega zeros(nx,1); zeros(1,nx) 1];
    sol = [X ones(nD,1)]'*Y;
    sol = pinv(H)*sol;
    clear X;
    alpha = sol(1:end-1);
...
elseif (nD>50000)
    ...
    Xe = [K OneK';OneK One] + inv(gam).*[omega zeros(typesize,1); zeros(1,typesize) 1];
    Ye = [KY;OneY];
    sol = pinv(Xe)*Ye;
    clear Xe; 
    alpha = sol(1:end-1);
...
testfeatures = kernel_matrix(svX,kernel_type,sig,Xt);
Yt = testfeatures'*alpha + b;
clear testfeatures;
