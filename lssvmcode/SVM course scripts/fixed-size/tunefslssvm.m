function [gam,sig] = tunefslssvm(nmodel,svX,L,estfunc,optfun,loss_type)
...
%Perform the csa/ds to get an initial starting point
if strcmp(model.kernel_type,'lin_kernel'),
    if strcmp(model.global_opt,'ds'),
        [par,fval] = dsp(rand(1,5),@(x)simanncostfun1(x,model,'crossvalidatefslssvm',costargs));
    elseif ~strcmp(model.weights,'wmyriad') && ~strcmp(model.weights,'whuber')
        [par,fval] = fscsa(rand(1,5),@(x)simanncostfun1(x,model,'crossvalidatefslssvm',costargs));
    else
        [par,fval] = fscsa(rand(2,5),@(x)simanncostfun1(x,model,'crossvalidatefslssvm',costargs));
...
elseif strcmp(model.kernel_type,'RBF_kernel')
    if strcmp(model.global_opt,'ds'),
        [par,fval] = dsp(rand(2,5),@(x)simanncostfun2(x,model,'crossvalidatefslssvm',costargs));
    elseif ~strcmp(model.weights,'wmyriad') && ~strcmp(model.weights,'whuber')
        [par,fval] = fscsa(rand(2,5),@(x)simanncostfun2(x,model,'crossvalidatefslssvm',costargs));
    else
        [par,fval] = fscsa(rand(3,5),@(x)simanncostfun2(x,model,'crossvalidatefslssvm',costargs));
...
elseif strcmp(model.kernel_type,'poly_kernel'),
    ...
    if ~strcmp(model.weights,'wmyriad') && ~strcmp(model.weights,'whuber')
        [par,fval] = simann(@(x)simanncostfun3(x,model,'crossvalidatefslssvm',costargs),[0.5;0.5;1],[-5;0.1;1],[10;3;1.9459],1,0.9,5,20,2);
    else
        [par,fval] = simann(@(x)simanncostfun3(x,model,'crossvalidatefslssvm',costargs),[0.5;0.5;1;0.2],[-5;0.1;1;eps],[10;3;1.9459;1.5],1,0.9,5,20,2);
