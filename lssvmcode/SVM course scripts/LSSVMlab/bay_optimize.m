...
elseif level==3,
  ...
  A = fminunc(@costL3, start_param, opties, model, type, nb);
...
function [model, A,B] = bay_optimize2(model,type,nb,bay)  
  ...
  gam_opt = fminunc(@costL2, abs(model.gam), opties, model, type, nb,bay);
...
%
  model = changelssvm(model,'kernel_pars',abs(sig2+1000*eps));
